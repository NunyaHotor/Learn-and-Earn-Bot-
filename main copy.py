...
def start_handler(message):
    ...
    if len(args) > 1 and args[1].startswith("REF"):
        ...
        records = sheet_manager.users_sheet.get_all_records()
        for row in records:
            if row.get("referral_code") == referrer_code:
                referrer_id = row.get("user_id")
                break
...
def answer_handler(call):
    ...
    if tokens > 0:
        ...
        bot.send_message(chat_id, "🔚 You've run out of tokens. Use '💰 Buy Tokens' to continue playing!", reply_markup=create_main_menu())
...
def buy_token_callback(call):
    ...
    pending_token_purchases[chat_id] = {"amount": amount, "price_cedis": price, "price_usd": TOKEN_PRICING[package_label]['price_usd'], "package": package_label}
    notify_admin_token_purchase(chat_id, pending_token_purchases[chat_id], "MTN MoMo or USDT")
    bot.send_message(chat_id, f"To buy {amount} tokens for GHS {price}, send payment via MTN MoMo or USDT and reply with your transaction ID.\n\n{PAYMENT_INFO}")
    bot.answer_callback_query(call.id)
...
def daily_reward_handler(message):
    ...
    bot.send_message(chat_id, "Back to main menu:", reply_markup=create_main_menu())
...
⏸️ <b>Games Paused:</b> {progress['games_paused']}
    """
    bot.send_message(chat_id, stats_message, reply_markup=create_main_menu())
...
⏭️ <b>Questions Until Bonus:</b> {progress['questions_until_bonus']}
    """
    bot.send_message(chat_id, progress_message, reply_markup=create_main_menu())
...
👥 Total Referrals: <b>{int(user.get('ReferralEarnings', 0))}</b>
    """
    bot.send_message(chat_id, referral_message, reply_markup=create_main_menu())
...
def leaderboard_handler(message):
    ...
    for i, user in enumerate(sorted_users, 1):
        leaderboard_message += f"{i}. {user['Name']} (@{user.get('Username', 'None')}) - {user['Points']} points\n"
    bot.send_message(chat_id, leaderboard_message, reply_markup=create_main_menu())
...
{ABOUT_US}
    """
    bot.send_message(chat_id, help_message, reply_markup=create_main_menu())
...
def feedback_input_handler(message):
    ...
    if message.text == "🔙 Cancel":
        ...
        bot.send_message(chat_id, "Feedback cancelled.", reply_markup=create_main_menu())
        return
    send_feedback_to_admin(chat_id, message.text)
    del user_feedback_mode[chat_id]
    bot.send_message(chat_id, "✅ Thank you for your feedback! Our team will review it.", reply_markup=create_main_menu())
...
def process_approve_token_purchase(message):
    ...
    for tx in pending_transactions:
        if tx['transaction_id'] == transaction_id:
            ...
            sheet_manager.log_token_purchase(user_id, f"APPROVED_{transaction_id}", amount, tx.get('payment_method', 'N/A'))
            ...
            return
    bot.send_message(chat_id, "❌ Transaction ID not found.", reply_markup=create_admin_menu())
...
def back_to_user_menu_handler(message):
    chat_id = message.chat.id
    bot.send_message(chat_id, "Returning to user menu...", reply_markup=create_main_menu())
...
def pause_game_handler(call):
    ...
    if chat_id in current_question:
        ...
        del current_question[chat_id]
        bot.send_message(chat_id, "⏸️ Game paused. Use 'General Quiz' or 'Zone Quiz' to resume.", reply_markup=create_main_menu())
        bot.answer_callback_query(call.id)
...
def resume_game_handler(call):
    ...
    if chat_id in paused_games:
        ...
        bot.send_message(chat_id, "No paused game found.", reply_markup=create_main_menu())
        bot.answer_callback_query(call.id)
